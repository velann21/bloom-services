...
type Cache interface {
	...
	SetWithTTL(ctx context.Context, key string, value interface{}, expiration time.Duration) (string, error)
...
func (redis Redis) GetTransactionFunc(ctx context.Context, key string, value []byte, expiration time.Duration) func(tx *redisV8.Tx) error {
	txf := func(tx *redisV8.Tx) error {
		...
		_, err = tx.TxPipelined(ctx, func(pipe redisV8.Pipeliner) error {
...
func (redis Redis) Delete(ctx context.Context, key string) error {
	IntCmd := redis.Client.Del(ctx, key)
	if IntCmd.Err() != nil {
		return IntCmd.Err()
	}
	return nil
