...
deploy_bloom_user_service:
	cd ./users-service/deployments/helm_charts && helm upgrade --install redis ./redis --atomic --timeout 5m0s -n bloomreach && helm upgrade --install bloom-user-services ./user-service --set image.tag=$(tag) --atomic --timeout 5m0s --force -n bloomreach
